                    ...
                    interiorTwo.map((carInterior) => (
                        <div key={carInterior.id}>
                            <Grid container sx={{mt: 2, boxShadow: '0px 1px 1px rgba(0, 0, 0, 0.16)', padding: '8px'}}>
                                <Grid item xs={8}>
                                    <Typography onClick={() => displayPreview(carInterior.id)} sx={{fontWeight: 400, fontSize: "16px", cursor: 'pointer'}}>{carInterior.name}</Typography>
                                </Grid>
                                <Boxes />    
                            </Grid>
                            <Box item xs={4} sx={{ display: `${selectedIntId === carInterior.id ? 'flex' : 'none'}`, boxShadow: '0px 1px 1px rgba(0, 0, 0, 0.16)', padding: '8px', alignItems: 'center', justifyContent: 'flex-end' }} >
             ...
             <div className="py-3">
                        ...
                        electricSystemOne.map((carElectric) => (
                            <div key={carElectric.id}>
                                <Grid key={carElectric.id} container sx={{mt: 2, boxShadow: '0px 1px 1px rgba(0, 0, 0, 0.16)', padding: '8px'}}>
                                    <Grid item xs={8}>
                                        <Typography onClick={() => displayPreviewTwo(carElectric.id)} sx={{fontWeight: 400, fontSize: "16px", cursor: 'pointer'}}>{carElectric.name}</Typography>
                                    </Grid>
                                    <Boxes />    
                                </Grid>
                ...
                <div className="mt-5 mb-4" style={{width: '100%'}}>
                    <div className="row">
                        ...
                        <div className="col-7">
                            <Button onClick={() => router.push('/inspection/stages/stage-eight')} variant="contained" size="small" fullWidth sx={{textTransform: 'none', py: 1, borderRadius: 2, fontSize: 12,  '&:hover': {
                                    color: '#fff',
                                },}}>
                                Save & Continue
                            </Button>
