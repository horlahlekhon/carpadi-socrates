...
import { under_hood } from "../../../../src/utils/temp-data";
...
export default function StageSeven() {
  ...
  const [underHood, setUnderHood] = useState(under_hood);
  const [selectedHoodId, setSelectedHoodId] = useState(null);
  const dispatch = useDispatch();

  useEffect(() => {
    const persistedRatings = JSON.parse(
      localStorage.getItem("stageseven-ratings")
    );

    if (persistedRatings !== null) setUnderHood(persistedRatings);
  }, []);
  ...
  const isDisabled = underHood.some((obj) =>
    Object.values(obj).some((values) => values === null)
  );

  //this function save the ratings value nd takes the user to the next stage
  const movetoNextStage = () => {
    localStorage.setItem("stageseven-ratings", JSON.stringify(underHood));
    dispatch(nextStage());
  };

  return (
    <InspectionLayout
      title="Inspection Stage 7"
      backgroundColor={"#000"}
      bodyHeight="90vh"
    >
