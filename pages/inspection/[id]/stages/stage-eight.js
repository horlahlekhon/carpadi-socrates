...
import {
  electrical_system_two,
  road_test_findings_one,
...
export default function StageEight() {
  ...
  const [electricSystemTwo, setElectricSystemTwo] = useState(
    electrical_system_two
  );
  ...
  return (
    <InspectionLayout title="Inspection Stage 8" backgroundColor={"#000"} bodyHeight="90vh">
            ...
            {roadTest.map((carRoadTest) => (
              <div key={carRoadTest.id}>
                ...
                >
                  <Grid item xs={8}>
                    <Typography
                      onClick={() => displayPreviewTwo(carRoadTest.id)}
                      sx={{
                        fontWeight: 400,
                        fontSize: "16px",
                        cursor: "pointer",
